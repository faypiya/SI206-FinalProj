...
def calculate_proportion():
    # Query the database to get the count of characters in each house
    result = query_function("SELECT house, COUNT(*) FROM characters WHERE house <> '' GROUP BY house")

    # Convert the query result to a dictionary
    house_count = dict(result)

    # Calculate the proportion of deceased characters in each house
    ...
    # Normalize each proportion by dividing it by the sum
...
def calculate_wand_material():
    # Query the join between characters and wands table
